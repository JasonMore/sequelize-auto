...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    ...
    if(self.sequelize.options.define && self.sequelize.options.define.prefix){
      schemaName = self.sequelize.options.define.prefix;
    }

    self.queryInterface.describeTable(table, schemaName).then(function(fields) {
  ...
  function mapForeignKeys(table, fn) {
    ...
    var schemaName = self.sequelize.config.database;
    
    if(self.sequelize.options.define && self.sequelize.options.define.prefix){
      schemaName = self.sequelize.options.define.prefix;
    }
    var sql = self.dialect.getForeignKeysQuery(table, schemaName);
